...
def get_resource_name_completion_list(cli_ctx, resource_type=None):
    def completer(prefix, action, parsed_args, **kwargs):  # pylint: disable=unused-argument
        ...
        return [r.name for r in get_resources_in_subscription(cli_ctx, resource_type=resource_type)]
    return completer
...
def get_enum_type(data):
    ...
    return CLIArgumentType(choices=CaseInsensitiveList(choices), type=_type)
